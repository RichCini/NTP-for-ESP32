...
void  loop()
{
  ...
  if (bTimeReceived == false)
  {
    ...
    if (nNtpDelay >= NTP_DELAY_COUNT)
    {
      ...
      // Send the ntp packet.
      IPAddress ipNtpServer(129, 6, 15, 30);  //time-c.nist.gov 129.6.15.30
    ...
    if (nNtpDelay == (NTP_DELAY_COUNT - 1))
    {
      ...
      if (Udp.parsePacket())
      {
        ...
        tvTimeValue.tv_sec = ((unsigned long)chNtpPacket[40] << 24) +       // bits 24 through 31 of ntp time
                             ...
                             (((70UL * 365UL) + 17UL) * 86400UL) +          // ntp to unix conversion (70*365 + 17)*86400 = 2208988800
                             ( (DST + TIME_ZONE) * 3600UL) +                // time zone adjustment
                             (ADJ_FACTOR);                                  // transport delay fudge factor (5)

        // Set the ESP32 rtc.
        err = settimeofday(&tvTimeValue, NULL);
        printLocalTime();
        //sprintf(chBuffer, "\nSetTOD return: %i", err);
        //Serial.println(chBuffer);
